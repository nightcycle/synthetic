...
export type TextFieldParameters = Types.TextBoxParameters & {
	LowerText: ParameterValue<string>?,
	LowerTextColor3: ParameterValue<Color3>?,
	Width: ParameterValue<UDim>?,
	CornerRadius: ParameterValue<number>?,
	CharacterLimit: ParameterValue<number>?,
	MaintainLowerSpacing: ParameterValue<boolean>?,
	FocusedBackgroundColor3: ParameterValue<Color3>?,
...
function Constructor(config: TextFieldParameters): TextField
	...
	local CharacterLimit = _import(config.CharacterLimit, nil)
	...
	-- local IconCount = _Computed(function(left, right)
	...
	local Focused: ValueState<boolean> = _Value(false)
	local IsFocused = Focused
	local TextBox = _new("TextBox")({
		...
		[_Fuse.Event "Focused"] = function()
			Focused:Set(true)
		end,
		[_Fuse.Event "FocusLost"] = function()
			Focused:Set(false)
		end,
	})
	TextBox = TextBox
	Focused:Set(TextBox:IsFocused())

	local Input = _Fuse.Property(TextBox, "Text")
	Input:Connect(function(cur)
		OnInputChanged:Fire(cur)
	end)
	Input = Input

	Focused:Connect(function(v)
		if v == false then
	...
	local Frame = _new "Frame" {
		...
		BackgroundColor3 = _Computed(function(isFocused, isHovering, backColor, focusColor, hoverColor)
			if isFocused then
				return focusColor
		...
		Children = {
				_new("Frame")({
					...
					BackgroundColor3 = _Computed(function(isFocused, borderCol, textCol)
						if isFocused then
							return borderCol
						else
							return textCol
				...
				_new "UIStroke" {
					...
					Color = _Computed(function(bCol, tCol, isFoc)
						if isFoc then
							return bCol
						else
							return tCol
					...
					Transparency = _Computed(function(trans)
						if trans == 0 then
							return 1
						else
							return 0
	...
	local Output: Frame = _new("Frame")(parameters)
	...
	local _setInput = Util.bindFunction(Output, _Maid, "SetInput", function(txt, cursorOffset: number?)
		...
		TextBox.CursorPosition += cursorOffset or 0
		return nil
	end)
	...
	_Computed(function(input, count: number, lim: number): nil
		if lim then
			if lim < count then
